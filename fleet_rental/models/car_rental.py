...
class CarRentalContract(models.Model):
    ...
    #                              )
    vehicle_id = fields.Many2one('fleet.vehicle', string="Vehicle", help="Vehicle",
                                 readonly=True,
                                 states={'draft': [('readonly', False)]}
                                 )
    ...
    move_ids = fields.Many2many('account.move', 'rental_move_rel', 'car_rental_id', 'move_id', string="Moves")
    # Change over - Varun
    ...
    def action_confirm(self):
        ...
        for line in self.checklist_line:
            gross_amount = gross_amount + (line.vehicle_rate * line.price)
...
class CarRentalChecklist(models.Model):
    ...
    price = fields.Float(string="Total Charges", compute='get_total_price')
    ...
    vehicle_rate = fields.Float("Vehicle Rate")
    rent_start_date = fields.Datetime("Rent Start Date")
    rent_end_date = fields.Datetime("Rent End Date")

    @api.multi
    @api.depends('rent_start_date', 'rent_end_date', 'vehicle_rate')
    def get_total_price(self):
        for rec in self:
            rec.price = 0.0
            if rec.rent_end_date and rec.rent_start_date:
                delta = rec.rent_end_date - rec.rent_start_date
                days = delta.days if delta else 0
                rec.price = rec.vehicle_rate * days
