...
class HelpdeskTicket(models.Model):
    ...
    def _get_default_stage_id(self):
        return self.env['custom_helpdesk.stage'].search([], order='sequence', limit=1)

    @api.model
    def _get_next_stage(self):
        return self.env['custom_helpdesk.stage'].search([], order='sequence', limit=2)[1].id
    ...
    team_id = fields.Many2one('custom_helpdesk.team', string='Support Team', track_visibility='onchange',
        default=lambda self: self.env['custom_helpdesk.team'].sudo()._get_default_team_id(user_id=self.env.uid),
    ...
    date_done = fields.Datetime(string='Ticket Completed On', track_visibility='onchange')
    category = fields.Many2one('custom_helpdesk.ticket_category',string="Ticket Category")

    stage_id = fields.Many2one('custom_helpdesk.stage', string='Stage', index=True, track_visibility='onchange',
                               domain="[]",
                               copy=False,
    ...
    assigned_time = fields.Datetime(string="Assigned On")
    take_up_time = fields.Datetime(string="Take up Time")
    ...
    def message_new(self, msg, custom_values=None):
        ...
        if custom_values:
            ...
            if team_id:
                team = self.env['custom_helpdesk.team'].sudo().browse(team_id)
                if team.company_id:
    ...
    def write(self, vals):
        ...
        if 'stage_id' in vals:
            ...
            stage = self.env['custom_helpdesk.stage'].browse(vals['stage_id'])
            if stage.last:
                vals.update({'date_done': fields.Datetime.now()})
            else:
                vals.update({'date_done': False})

        #check if assigned user changes
        if 'user_id' in vals:
            #notify user about ticket
            user = self.env['res.users'].search([('id','=',vals['user_id'])])

            self.message_post(
                   body="A new service desk ticket as been assigned to you",
            ...
            #update assinged time
            vals.update({'assigned_time':fields.Datetime.now()})
    ...
    def completed(self):
        self.ensure_one()
        vals = {
           'stage_id':self.env['custom_helpdesk.stage'].search([], order='sequence', limit=3)[2].id,
    ...
    def takeit(self):
        ...
        vals = {
            ...
            'take_up_time':fields.Datetime.now(),
            'stage_id':self._get_next_stage(),
            # 'team_id': self.env['custom_helpdesk.team'].sudo()._get_default_team_id(user_id=self.env.uid).id
        }
        return super(HelpdeskTicket, self).write(vals)
    ...
    def _register_hook(self):
        ...
        if HelpdeskTicket.website_form:
            ...
            self.env['ir.model.fields'].formbuilder_whitelist(
                self._name, ['name', 'description', 'date_deadline', 'priority', 'partner_id', 'user_id','contact_name','email_from'])
        pass




class Custom_helpdesk_category(models.Model):
    _name="custom_helpdesk.ticket_category"
