...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        ticket_count = request.env['custom_helpdesk.ticket'].search_count([])
        values.update({
...
#             'stage': {'input': 'stage', 'label': _('Search in Stages')},
        ...
        archive_groups = self._get_archive_groups('custom_helpdesk.ticket', domain)
        ...
        ticket_count = request.env['custom_helpdesk.ticket'].search_count(domain)
        # pager
        ...
        helpdesk_ticket = request.env['custom_helpdesk.ticket'].search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
        ...
        return request.render("custom_helpdesk.portal_my_tickets", values)

    @http.route(['/my/tickets/<int:ticket_id>'], type='http', auth="user", website=True)
    def my_tickets_ticket(self, ticket_id=None, **kw):
        ticket = request.env['custom_helpdesk.ticket'].browse(ticket_id)
        return request.render("custom_helpdesk.my_tickets_ticket", {'ticket': ticket})

    @http.route(['/helpdesk/new'], type='http', auth="public", website=True)
    def ticket_new(self, **kw):
        pri = request.env['custom_helpdesk.ticket'].fields_get(allfields=['priority'])['priority']['selection']
        pri_default = '1'
        ...
        return request.render("custom_helpdesk.new_ticket", vals)
