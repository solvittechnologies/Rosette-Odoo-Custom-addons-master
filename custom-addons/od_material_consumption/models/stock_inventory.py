...
class StockInventoryLine(models.Model):
    ...
    def _compute_product_quantity_value(self):
        for rec in self:
            stock_quant_obj = self.env['stock.quant'].search([('location_id','=',rec.location_id.id),('product_id','=',rec.product_id.id)])
            if len(stock_quant_obj) >0:
                rec.product_quty = stock_quant_obj.quantity
            else:
                rec.product_quty = 0
