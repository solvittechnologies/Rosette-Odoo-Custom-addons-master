...
class SupportTeam(models.Model):
    ...
    def _get_default_team_id(self, user_id=None):
        ...
        if 'default_team_id' in self.env.context:
            team_id = self.env['custom_helpdesk.team'].browse(self.env.context.get('default_team_id'))
        if not team_id or not team_id.exists():
            team_id = self.env['custom_helpdesk.team'].sudo().search(
                ['|', ('user_id', '=', user_id), ('member_ids', '=', user_id)],
                limit=1)
        if not team_id:
            team_id = self.env.ref('custom_helpdesk.team_alpha', raise_if_not_found=False)
        return team_id


    def get_alias_model_name(self, vals):
        return 'custom_helpdesk.ticket'
