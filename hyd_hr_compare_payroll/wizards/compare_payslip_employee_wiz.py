...
class TableauComparatifPaieWizard(models.TransientModel):
    _name = 'hyd_hr_compare_payroll.compare_payslip_employee_wiz'
    ...
    employees = fields.Many2many(
        ...
        relation="tabcompar_to_employee")

    select_rules = fields.Selection(
        string="Filter",
        selection=[
            ('appears', 'Appears on payslip'),
            ('all_rules', 'All salary rules'),
            ('select', 'Select rules')],
        default='appears',
        required=True)
    ...
    rules = fields.Many2many(
        ...
        relation="compar_payroll_emp_to_salary_rule")
    ...
    def print_report(self):
        ...
        res['all_employee'] = self.all_employee
        res['employees'] = self.employees.mapped('id')
        res['select_rules'] = self.select_rules
        res['rules'] = self.rules.mapped('id')
        datas['form'] = res

        report_name = 'hyd_hr_compare_payroll.compare_payslip_employee_report'
